...
select2add();
 function select2add() {
  ...
  add();
  function add() {
    $('#form_user').load('<?= base_url("transaksi_faktur/add")?>');
  }

  function open_edit(id) {
    $('#content').load('<?= base_url("transaksi_faktur/edit/")?>'+id);

  }

  function open_state(id) {
    $('#content').load('<?= base_url("transaksi_faktur/state/")?>'+id);
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('transaksi_faktur/hapus')?>",
             type: 'POST',
             data: {id:id},            
...
$("#form_add").submit(function(e) {
         e.preventDefault();
         $.ajax({
          url: "<?= base_url('transaksi_faktur/add')?>",
             type: 'POST',
             data: $(this).serialize(),             
             success: function(data) {    
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('transaksi_faktur/restore')?>",
             type: 'POST',
             data: {id:id},            
             success: function(data) {  
              setTimeout(function() {
