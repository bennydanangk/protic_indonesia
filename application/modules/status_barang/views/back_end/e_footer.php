...
select2add();
 function select2add() {
  ...
  content();
  function content() {
    $('#content').load('<?= base_url("status_barang/content")?>')
  }
  function data_sampah() {
    $('#content').load('<?= base_url("status_barang/data_sampah")?>')
  }



  function add() {
    $('#content').load('<?= base_url("status_barang/add")?>');
  }

  function open_edit(id) {
    $('#content').load('<?= base_url("status_barang/edit/")?>'+id);

  }

  function open_state(id) {
    $('#content').load('<?= base_url("status_barang/state/")?>'+id);
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('status_barang/hapus')?>",
             type: 'POST',
             data: {id:id},            
...
$("#form_add").submit(function(e) {
         e.preventDefault();
         $.ajax({
          url: "<?= base_url('status_barang/add')?>",
             type: 'POST',
             data: $(this).serialize(),             
             success: function(data) {    
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('status_barang/restore')?>",
             type: 'POST',
             data: {id:id},            
             success: function(data) {  
              setTimeout(function() {
