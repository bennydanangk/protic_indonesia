...
public function CreateCode($kd){
  $this->db->select('RIGHT(data_barang.kode_barang,5) as kode_barang', FALSE);
...
function ambil_barang($table,$where)  {
  $this->db->join('t_jenis', 'data_barang.id_jenis = t_jenis.id_jenis', 'left');
  $this->db->join('t_satuan', 'data_barang.id_satuan = t_satuan.id_satuan', 'left');
  $this->db->join('t_kategori', 'data_barang.id_jenis = t_kategori.id_kategori', 'left');
  $this->db->join('t_user', 't_user.id_user = data_barang.id_user_input', 'left');
