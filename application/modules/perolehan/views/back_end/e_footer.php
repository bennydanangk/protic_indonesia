...
select2add();
 function select2add() {
  ...
  content();
  function content() {
    $('#content').load('<?= base_url("perolehan/content")?>')
  }
  function data_sampah() {
    $('#content').load('<?= base_url("perolehan/data_sampah")?>')
  }



  function add() {
    $('#content').load('<?= base_url("perolehan/add")?>');
  }

  function open_edit(id) {
    $('#content').load('<?= base_url("perolehan/edit/")?>'+id);

  }

  function open_state(id) {
    $('#content').load('<?= base_url("perolehan/state/")?>'+id);
...
$("#form_add").submit(function(e) {
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('perolehan/restore')?>",
             type: 'POST',
             data: {id:id},            
             success: function(data) {  
              setTimeout(function() {
