...
select2add();
 function select2add() {
  ...
  content();
  function content() {
    $('#content').load('<?= base_url("Opd_ruangan/content")?>')
  }
  function data_sampah() {
    $('#content').load('<?= base_url("Opd_ruangan/data_sampah")?>')
  }



  function add() {
    $('#content').load('<?= base_url("Opd_ruangan/add")?>');
  }

  function open_edit(id) {
    $('#content').load('<?= base_url("Opd_ruangan/edit/")?>'+id);

  }

  function open_state(id) {
    $('#content').load('<?= base_url("Opd_ruangan/state/")?>'+id);
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('opd_ruangan/hapus')?>",
             type: 'POST',
             data: {id:id},            
...
$("#form_add").submit(function(e) {
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('opd_ruangan/restore')?>",
             type: 'POST',
             data: {id:id},            
             success: function(data) {  
              setTimeout(function() {
