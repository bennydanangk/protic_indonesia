...
select2add();
 function select2add() {
  ...
  content();
  function content() {
    $('#content').load('<?= base_url("barang_faktur/content")?>')
  }
  function data_sampah() {
    $('#content').load('<?= base_url("barang_faktur/data_sampah")?>')
  }



  function add() {
    $('#content').load('<?= base_url("barang_faktur/add")?>');
  }

  function open_edit(id) {
    $('#content').load('<?= base_url("barang_faktur/edit/")?>'+id);

  }

  function open_state(id) {
    $('#content').load('<?= base_url("barang_faktur/state/")?>'+id);
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('barang_faktur/hapus')?>",
             type: 'POST',
             data: {id:id},            
...
$("#form_add").submit(function(e) {
         e.preventDefault();
         $.ajax({
          url: "<?= base_url('barang_faktur/add')?>",
             type: 'POST',
             data: $(this).serialize(),             
             success: function(data) {    
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('barang_faktur/restore')?>",
             type: 'POST',
             data: {id:id},            
             success: function(data) {  
              setTimeout(function() {
