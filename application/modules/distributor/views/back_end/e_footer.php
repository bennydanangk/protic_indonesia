...
select2add();
 function select2add() {
  ...
  content();
  function content() {
    $('#content').load('<?= base_url("distributor/content")?>')
  }
  function data_sampah() {
    $('#content').load('<?= base_url("distributor/data_sampah")?>')
  }



  function add() {
    $('#content').load('<?= base_url("distributor/add")?>');
  }

  function open_edit(id) {
    $('#content').load('<?= base_url("distributor/edit/")?>'+id);

  }

  function open_state(id) {
    $('#content').load('<?= base_url("distributor/state/")?>'+id);
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('distributor/hapus')?>",
             type: 'POST',
             data: {id:id},            
...
$("#form_add").submit(function(e) {
  ...
  if (result.isConfirmed) {
    ...
    $.ajax({
    url: "<?= base_url('distributor/restore')?>",
             type: 'POST',
             data: {id:id},            
             success: function(data) {  
              setTimeout(function() {
